...
import numpy as np
import torch
from tcn.data_provider import datasets_factory
from tcn.models.model_factory import Model
from tcn.utils import preprocess, logger
import tcn.trainer
import datetime
    ...
    for itr in range(1, args.max_iterations + 1):
        ...
        if itr % args.test_interval == 0:  # 1000
            avg_mse, ssim, psnr, fmae, sharp = trainer.test(
                model, test_input_handle, args, itr)
        ...
        if itr % args.loss_interval == 0:  # 10000
            fileName = "/loss iter{}".format(itr) + \
                datetime.datetime.now().strftime('date:' + '%Y_%m_%d')
...
def test_wrapper(model, args):
    ...
    avg_mse, ssim, psnr, fmae, sharp = trainer.test(
        model, test_input_handle, args, 'test_result')
    fileName = "/test loss " + datetime.datetime.now().strftime('date:' + '%Y_%m_%d')
    np.savez_compressed(args.loss_dir + fileName, avg_mse=avg_mse,
                        ssim=ssim, psnr=psnr, fmae=fmae, sharp=sharp)
...
def make_args(parser):
    ...
    parser.add_argument('--save_dir', type=str,
                        default='moving-mnist/checkpoints/mnist_tctn')
    parser.add_argument('--gen_frm_dir', type=str,
                        default='moving-mnist/results/mnist_tctn')
    parser.add_argument('--loss_dir', type=str,
                        default='moving-mnist/loss/mnist_tctn')

    # the print content save path, ect training loss
    parser.add_argument('--print_path', type=str,
                        default='moving-mnist/loss/mnist_tctn')
    ...
    parser.add_argument('--n_steps', type=int, default=50000,
                        help='number of iteration to update 2 times learning rate')
    ...
    parser.add_argument('--model_depth', type=int,
                        default=128, help='depth of the model')
    parser.add_argument('--de_layers', type=int, default=6,
    ...
    parser.add_argument('--n_heads', type=int, default=1,
                        help='number of heads in conv mult-ihead attention')
    parser.add_argument('--dec_frames', type=int, default=19,
                        help='nummber of output frames')
    parser.add_argument('--w_res', type=bool, default=True,
                        help='using residual connect or not')
    parser.add_argument('--w_pos', type=bool, default=True,
                        help='using positional encoding or not')
    parser.add_argument('--pos_kind', type=str, default='sine',
                        help='kind of positional encoding,two choice: sine,learned')
    parser.add_argument('--model_type', type=int, default=1,
                        help='type of the model, 0 for interpoation model and 1 for extrapolation model')
